...
public struct GTFSCSV {
    ...
    public enum CSVError: Error, LocalizedError {
        ...
        public var errorDescription: String? {
            ...
            case .missingColumns(let wanted, let available):
                "Missing column(s) " + wanted.joined(separator: ", ")
                    + "; file has " + available.joined(separator: ", ")
            }
        }
    }
    ...
    public struct Row {
        ...
        public func string(_ index: Int) -> String {
            ...
            guard field.end > field.start, let base = bytes.baseAddress else { return "" }
            let slice = UnsafeBufferPointer(start: base + field.start, count: field.end - field.start)
        ...
        public func int(_ index: Int) -> Int? {
            ...
            var index = field.start
            var negative = false
            if bytes[index] == 0x2D { negative = true; index += 1 }
            guard index < field.end else { return nil }
            while index < field.end {
    ...
    private static func scanRow(
        ...
        while index <= buffer.count {
            if index < buffer.count, buffer[index] == 0x22 {  // '"'
                index = scanQuotedField(buffer, from: index, into: &fields)
            } else {
                index = scanBareField(buffer, from: index, into: &fields)
            }

            if index >= buffer.count { return buffer.count }
            if buffer[index] == 0x2C { index += 1; continue }   // ',' -> another field
            if buffer[index] == 0x0D { index += 1 }             // CR of a CRLF
            if index < buffer.count, buffer[index] == 0x0A { index += 1 }
            return index
        }
        return buffer.count
    }

    /// A quoted field ends at the first quote not doubled. Anything between that
    /// quote and the delimiter is malformed and skipped, rather than swallowing the
    /// rest of the file.
    private static func scanQuotedField(
        _ buffer: UnsafeBufferPointer<UInt8>,
        from offset: Int,
        into fields: inout [Field]
    ) -> Int {
        ...
        while index < buffer.count {
            if buffer[index] == 0x22 {
                if index + 1 < buffer.count, buffer[index + 1] == 0x22 {
                    hasEscapes = true
                    index += 2
                    continue
                }
        ...
        fields.append(Field(start: contentStart, end: min(index, buffer.count), hasEscapes: hasEscapes))
        if index < buffer.count { index += 1 }   // closing quote
        while index < buffer.count, buffer[index] != 0x2C, buffer[index] != 0x0A, buffer[index] != 0x0D {
            index += 1
        }
        return index
    }

    private static func scanBareField(
        _ buffer: UnsafeBufferPointer<UInt8>,
        from offset: Int,
        into fields: inout [Field]
    ) -> Int {
        ...
        fields.append(Field(start: contentStart, end: index, hasEscapes: false))
        return index
    }
