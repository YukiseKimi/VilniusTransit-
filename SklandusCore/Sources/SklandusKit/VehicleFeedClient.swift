...
public actor VehicleFeedClient {
    ...
    public func stop() {
        pollTask?.cancel()
        pollTask = nil
        Task { [reachability] in await reachability.stop() }
    }
    ...
    private var isOffline: Bool {
        get async { await reachability.isOnline == false }
    }

    /// Pausing on a dead link avoids burning the backoff budget on requests that
    /// cannot succeed, and makes the app resume as soon as the link returns.
    private func startPathMonitor() {
        Task { [reachability] in await reachability.start() }
    }
}
